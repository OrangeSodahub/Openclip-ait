...
__global__ void
fused_elementwise_176(uint4* output0, const uint4* input0,const uint4* input1,  int n_elements) {
  ...
  input_tmp1 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ uint4,
                                     /*is_contiguous*/ true>(
      input_tmp1, (idx * N_ELEMENTS_PER_THREAD % (1 * 77 * 512) / (1) * (1)) / N_ELEMENTS_PER_THREAD, 0, 0, 0);
...
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = __hadd2(p_tmp_i0[i],p_tmp_i1[i]);
...
void invoke_fused_elementwise_176(half* output0, const half* input0,const half* input1,  int n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_176<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
